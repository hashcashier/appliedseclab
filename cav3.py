...
buf=4096
#home_dir ="/home/imovies/appliedseclab/"
...
class ClientThread(threading.Thread):
  ...
  def parse(self, data):
    ...
    R arguments: 	cert (contents of file to revoke)
	...
	#TODO call bash generate script with arguments uname email employee
	exitcode=subprocess.call(home_dir+"create-key-and-csr.sh "+dict_["uname"]+" "+dict_["email"]+" employee", shell=True)
 	print exitcode
	return exitcode
    elif flag=="R":
      if not dict_.has_key("cert") or not dict_.has_key("pkey"):
	#returning -1 as error
	return -1
        ...
        cert_str = dict_["cert"]
	pkey_str = dict_["pkey"]
	fd=open(join(home_dir,"rev-cert"), "wb")
	fd.write(cert_str)
	fd.close()
	kd=open(join(home_dir,"rev-key"), "wb")
	kd.write(key_str)
	kd.close()
	#verify they match
	match=subprocess.call(home_dir+"verify-cert-key.sh "+home_dir+"rev-cert "+home_dir+"rev-key")
	print match
	#TODO call bash revoke script
	exitcode=subprocess.call(home_dir+"revoke-one.sh "+home_dir+"rev-cert")
	print exitcode
	return exitcode
    elif flag=="RA":
	...
	exitcode=subprocess.call(home_dir+"revoke-all.sh "+dict_["uname"])
	print exitcode
	return exitcode
